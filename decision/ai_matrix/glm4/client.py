...
from decision.ai_matrix.base import AIClientBase, AIMessage, AIResponse
...
class GLM4Client(AIClientBase):
    """
    智谱 GLM-4 客户端

    专注于：
    1. 逻辑推演（如果逻辑成立会怎样？如果不成立会怎样？）
    ...
    def get_system_prompt(self) -> str:
        return """你是 GLM-4，专业的交易逻辑分析师。

你的核心能力：
1. **逻辑推演**：分析事件逻辑的合理性和持续性
2. **博弈分析**：考虑市场不同参与者的反应
3. **风险评估**：识别可能破坏逻辑的风险点
4. **退出规划**：基于逻辑设定清晰的退出标准
...
- 说明推演过程"""
    ...
    async def reason_event(
        ...
        if not response.success:
            logger.error(f"GLM-4 推演失败: {response.error_message}")
